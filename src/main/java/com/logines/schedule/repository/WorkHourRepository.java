...
public interface WorkHourRepository extends JpaRepository<WorkHour, Integer> {
    ...
    @Query(value = "SELECT * FROM logines.work_hour WHERE TO_TIMESTAMP(s.start_time,  'YYYY-MM-DD HH24:mi') BETWEEN :start_time::timestamp AND now()::timestamp", nativeQuery = true)
