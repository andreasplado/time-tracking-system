...
public interface WorkHourRepository extends JpaRepository<WorkHour, Integer> {
    ...
    @Query(value = "SELECT s.* FROM logines.work_hour s WHERE s.start_time\\:\\:timestamp between ? and NOW()\\:\\:timestamp", nativeQuery = true)
    ...
    List<WorkHour> findWetweenTime(String startTime, String endTime);
