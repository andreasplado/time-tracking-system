...
public interface WorkHourRepository extends JpaRepository<WorkHour, Integer> {
    ...
    @Query(value = "SELECT * FROM logines.work_hour WHERE TO_TIMESTAMP(start_time,  'YYYY-MM-DD HH24:mi')\\:\\:timestamp BETWEEN TO_TIMESTAMP(:start_time, 'YYYY-MM-DD HH:mi')\\:\\:timestamp AND now()\\:\\:timestamp", nativeQuery = true)
